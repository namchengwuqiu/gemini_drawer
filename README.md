...
- **异步任务**：创建后使用响应中的 `video_id` 和对应 `model_name` 查询 `/agnesapi`；只有 `status=completed` 时才取 `metadata.url` 下载和发送。正常每 2 秒查询一次，临时网络错误、429 和 5xx 会退避重试同一任务，不重新提交。生成阶段总上限为 420 秒，为宿主 600 秒的视频超时留出下载和发送时间；本地超时不代表服务端任务已取消；提交结果不确定或生成超时后停止自动换 Key/渠道重提，也不会把在途任务误计为 Key 失效。
